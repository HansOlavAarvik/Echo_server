...
def calculate_db():
    ...
    try:
        time = datetime.now()
        samples = np.frombuffer(audio_buffer[-1000,-1], np.int16)
        ...
        for key in KEYS_LIST:
            db_data[key] = validate_and_extract(db, key, 1)
        return add_data(db_data)
    except:
        log("samples out of range for db calc")
        return  np.nan